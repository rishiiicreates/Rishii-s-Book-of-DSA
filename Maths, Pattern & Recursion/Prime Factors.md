...
- **the Algorithm:**
- **strip all 2s:** Even numbers are common. Divide $N$ by 2 as long as it's even, storing `2` each time. This makes $N$ entirely odd.
- **search up to $\sqrt{N}$:** Now loop from $i = 3$ up to $\sqrt{N}$, incrementing by 2 (checking only odd numbers: 3, 5, 7...).
- **divide greedily:** While $N$ is divisible by $i$, push $i$ to our list and divide $N$ by $i$. Because we divide greedily, we will never accidentally push a composite number (like 9), because its prime factors (like 3) were already completely stripped out!
- **the final prime:** If after the loop, $N$ is still greater than 2, then $N$ itself is a massive prime number (the single factor that was greater than $\sqrt{N}$). We push it to the list.
