...
- if $N$ is negative, or if it ends in 0 (but isn't exactly 0), it can't be a palindrome.
- we extract digits from the back and construct a `reversedHalf` number.
- we know we've reached the middle when the original number $N$ becomes $\le$ `reversedHalf`.
