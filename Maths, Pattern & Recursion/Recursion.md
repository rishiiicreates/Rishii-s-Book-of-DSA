...
- **the Base Case(s):** The terminating condition. This is the simplest instance of the problem that can be answered directly without further recursive calls. Without a base case, the function will loop infinitely.
- **the Recursive Step:** The part of the function that breaks the current problem down and makes a call to itself with modified arguments, driving the problem closer to the base case.
...
- `printCountdown(3)` executes, prints `3`, and calls `printCountdown(2)`. The function is *paused*.
- a new frame is pushed for `printCountdown(2)`. It prints `2`, calls `printCountdown(1)`.
- a new frame is pushed for `printCountdown(1)`. It prints `1`, calls `printCountdown(0)`.
- `printCountdown(0)` hits the base case, prints `"Liftoff!"` and `return`s.
- the stack frames then sequentially *pop* off the stack, resuming and instantly finishing the paused parent functions.
