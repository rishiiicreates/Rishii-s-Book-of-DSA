...
- **outer Loop:** Controls the rows from $i = 1$ to $N$.
- **inner Loop:** In a right-angled triangle, the $i$-th row has exactly $i$ elements. So the inner loop runs from $j = 1$ to $i$.
- **state Variable:** We define a variable `count = 1` *outside* both loops. Inside the inner loop, we print `count`, and immediately increment it `count++`.
