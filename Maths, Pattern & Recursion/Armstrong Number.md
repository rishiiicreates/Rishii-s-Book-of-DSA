...
- **count the digits:** First, we need to know the power $K$ to raise each digit to. We can use the logarithmic $O(1)$ approach from [[Count Digits]].
- **extract digits:** We can extract the last digit of a number using `N % 10`.
- **truncate:** We remove the last digit from the number using `N / 10`.
- **sum and Compare:** We accumulate $digit^K$ into a sum and compare it to the original number.
