...
1. **Base Case:** If $N == 0$, return back.
2. **Recursive Step:** Call `print1toN(N - 1)`.
3. **Execution:** Only *after* the base case is hit and the frames start popping off the Call Stack do we execute `cout << N`. This ensures the smallest numbers print first.
