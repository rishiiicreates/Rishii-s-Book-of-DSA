...
- recursively penetrate the rightward subspace strictly (approaching absolute maximums).
- as geometry returns towards roots, algebraically accumulate: $S = S + V_{original}$.
- topologically mutate the current node's internal state explicitly to the evaluated state of $S$.
- pass the temporal causality $S$ uniformly penetrating down the leftward descending sequence.
