...
- we construct a topological In-Order DFS recursive descent.
- we inject a global state tracker `count = 0` and an algebraic target `ans = -1`.
- the absolute moment the traversal evaluates the current vertex ($T$), we mutate `count++`.
- if `count == K`, we have successfully mapped the target. Log `ans = T.val` and propagate a short-circuit return to immediately collapse the remaining execution stack.
