...
- **base Constraint:** If the current `node` evaluates to `nullptr`, instantiate a new node with `val` and return its memory address.
- **recursive Pathing:**
...
- **return Subtree Root:** Return the current `node` to maintain link continuity up the recursive call stack.
