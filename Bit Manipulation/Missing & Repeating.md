...
- **combined XOR:** Let $S$ be the XOR sum of all elements in $A$ *and* all integers from $1$ to $N$.
...
- **bit Isolation:** Isolate the rightmost set bit of $S$ using `mask = S & -S`. This bit represents a position where $X$ and $Y$ differ.
- **partition and Resolve:** Iterate through both the array $A$ and the sequence $1 \dots N$. Partition all these numbers into two sets based on the isolated bit.
...
- **distinguish:** Iterate through $A$ to check which of the two isolated numbers actually exists in the array. That one is $Y$ (the repeating number). The other is $X$ (the missing number).
