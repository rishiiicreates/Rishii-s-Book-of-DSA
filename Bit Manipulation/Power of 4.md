...
- **positivity:** $N > 0$. Negative numbers and $0$ are structurally invalid.
- **singular Set Bit:** $N$ must be a power of $2$. A power of $2$ contains exactly one `1` in its binary representation. This is validated by $N \ \& \ (N - 1) == 0$.
- **even Bit Position:** In a 0-indexed binary representation, powers of $4$ exclusively populate the even indices:
