...
- first, count the number of spaces in the "true" portion of the string to calculate the exact final length of the string: `final_length = true_length + spaces * 2`.
- use two pointers: one pointing to the end of the true string (`i = true_length - 1`), and one pointing to the end of the newly calculated final string (`index = final_length - 1`).
- iterate `i` backwards. If `str[i]` is a space, we write `%20` at the `index` and decrement `index` by 3. Otherwise, we copy `str[i]` to `str[index]` and decrement `index` by 1.
