...
- **the Magic Constant:** The sum of digits 1-9 is 45. Since there are 3 rows, each row must mathematically sum to exactly $45 / 3 = 15$.
- **the Center Cell:** The center cell belongs to one row, one column, and both diagonals. The only mathematical way to arrange the digits 1-9 such that all symmetric lines cross the center and sum to 15 is if the **center cell is strictly 5**.

- algorithm:
- iterate through all possible top-left corners of a $3 \times 3$ subgrid.
- pruning: If the center cell (`grid[r+1][c+1]`) is not 5, skip immediately.
- verification: Check if the grid contains all digits 1-9 exactly once.
- validation: Verify that all rows, columns, and diagonals sum to 15.
