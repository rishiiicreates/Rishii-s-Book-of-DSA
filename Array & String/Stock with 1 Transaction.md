...
1. Track `minPrice` initialized to infinity.
2. Track `maxProfit` initialized to $0$.
3. As you iterate day by day, update `minPrice`.
4. The potential profit on day $i$ is `prices[i] - minPrice`. Maximize `maxProfit` with this value.
