---
type: concept
tags: [heap, priority-queue, array, cpp]
date: 2026-06-30
---
...
1. **Heapify-Up (Percolate Up):** Invoked during `insert()`. A new element is appended to the absolute end of the array (preserving the Complete Binary Tree property). If this node violates the Min Heap order ($Node < Parent$), it is mathematically swapped upwards until the invariant is restored.
2. **Heapify-Down (Percolate Down):** Invoked during `extractMin()`. The root (minimum element) is overwritten by the last element in the array, and the array shrinks. Because this new root likely violates the Min Heap order ($Node > Children$), it is mathematically swapped downwards with its *smallest child* until the invariant is restored.
