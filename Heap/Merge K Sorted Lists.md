...
- initialization mathematically inserts exactly one pointer bounding each discrete list ($K$ elements) directly into the Min-Heap.
- iterative Sweep:
...
- the geometric structure strictly sustains absolute size $K$, implicitly evaluating bounding minima in strict $\mathcal{O}(\log K)$ logarithmic geometry.
