---
type: concept
tags: [heap, priority-queue, array, cpp, hashing, sort]
date: 2026-07-01
---
...
1. The absolute minimum scalar in the array mathematically receives a rank of 1.
2. If two elements map to strict equality ($A[i] = A[j]$), they must share the identical topological rank.
3. The absolute rank increments strictly monotonically by 1 ($\text{Rank}_{k} = \text{Rank}_{k-1} + 1$) for sequentially larger unique scalars.
4. The output array $R$ must structurally mirror the topological geometry of the input array $A$ such that $R[i] = \text{Rank}(A[i])$.
...
1. Construct a topological clone of $A$ and sort it in strict monotonic ascending order.
2. Iterate the sorted boundary array. If the current scalar does not mathematically exist in the Hash Map, bind it to the current absolute `rank` and mutate `rank++`.
3. Geometrically iterate the original array $A$, substituting $A[i]$ with its mapped constant time $O(1)$ rank lookup.
