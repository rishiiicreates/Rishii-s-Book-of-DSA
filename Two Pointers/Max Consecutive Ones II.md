...
- algorithm:
- initialize a spatial window defined by pointers $L = 0$ and $R = 0$.
- maintain a scalar state variable `zero_count` to mathematically track the structural violations within the window.
- iteratively expand the window by incrementing $R$. If `arr[R] == 0`, increment `zero_count`.
- **boundary Violation Constraint:** If `zero_count > 1`, the mathematical invariant is broken. We must forcefully shrink the window from the left by incrementing $L$. If `arr[L] == 0` during this contraction, decrement `zero_count`.
- the maximum length dynamically bounded by $R - L + 1$ is globally tracked whenever the invariant strictly holds (`zero_count <= 1`).
