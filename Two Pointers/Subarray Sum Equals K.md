---
type: concept
tags: [prefix-sum, hashing, cpp, math, array]
date: 2026-06-30
---
...
Algorithm:
1. Iteratively accumulate the prefix sum $S_i$.
2. For each iteration, compute the mathematical complement: $C = S_i - K$.
3. Query a Hash Map (`unordered_map<int, int>`) tracking the strict absolute frequencies of all historically encountered prefix sums. If $C$ exists in the map, add its structural frequency to our global valid subarray count.
4. Finally, record the current spatial prefix sum $S_i$ into the Hash Map.
