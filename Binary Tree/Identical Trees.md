...
- if both structural roots are geometrically null ($\emptyset$), they are trivially identical.
- if exactly one root is null, the geometric topology breaks immediately.
- if both nodes exist, their scalar integer values must be mathematically equivalent ($P.\text{val} == Q.\text{val}$).
- concurrently, their entire left disjoint subtrees must be identical, AND their entire right disjoint subtrees must be identical.
