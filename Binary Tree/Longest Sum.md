...
- geometric Length (Depth $D$) - Strictly the primary structural constraint.
- algebraic Sum (Sum $S$) - The secondary tie-breaking constraint.
...
- algorithm:
- construct a rigorous DFS invariant tracking temporal state: `(currentNode, currentDepth, currentSum)`.
- upon evaluating an absolute leaf node (both bounds are strictly `nullptr`), execute an algebraic update against the global optimum:
...
- penetrate dynamically into left and right subspaces, incrementing structural depth and algebraic sum.
