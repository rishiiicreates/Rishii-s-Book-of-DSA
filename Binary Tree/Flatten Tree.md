---
type: concept
tags: [binary-tree, cpp, linked-list, morris-traversal, inplace]
date: 2026-07-01
---
...
1. The mathematically last node evaluated in $T_L$'s Pre-Order traversal is explicitly its absolute rightmost geometric descendant $v_{R\_max}$.
2. The sequence dictates that $v_{R\_max}$ must topological point to $v$'s original right subtree $T_R$.
3. We architect a temporary topological bridge: map $v_{R\_max} \to \text{right} = v \to \text{right}$.
4. We surgically shift the entire left bound: map $v \to \text{right} = v \to \text{left}$, and irrevocably sever $v \to \text{left} = \emptyset$.
5. Advance the cursor strictly to the geometric right and repeat.
