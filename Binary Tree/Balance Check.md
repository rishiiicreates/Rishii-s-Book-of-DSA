...
- for any node:
- compute the structural height of the Left child. If it evaluates to `-1`, aggressively short-circuit and propagate `-1` up.
- compute the structural height of the Right child. If it evaluates to `-1`, propagate `-1`.
- if both subtrees are geometrically valid, compute the topological absolute difference $\Delta = |H_L - H_R|$.
- if $\Delta > 1$, trigger structural failure (`-1`). Otherwise, propagate the mathematically sound maximum height bound $1 + \max(H_L, H_R)$.
