...
- **base Constraint:** If `node` is `nullptr`, terminate.
- **validation Logic (Parent Perspective):**
...
- **recursive Descent:** Proceed to recursively analyze `node->left` and `node->right` via standard [[Depth First Search]].
- **post-Processing (Sorting):** The problem strictly requests the output in sorted order. Once the DFS exhausts the entire tree and populates our result vector, we apply $O(K \log K)$ sorting (where $K$ is the number of isolated nodes) to satisfy the constraint. If no such nodes exist, return `[-1]` by conventional problem definitions.
