...
- we propagate the absolute mathematical extrema: `current_max` and `current_min` observed *thus far* along the structural path.
- at any generic vertex $v$, the maximum divergence involving $v$ and any valid ancestor is exactly $\max(|v.\text{val} - \text{current\_max}|, |v.\text{val} - \text{current\_min}|)$.
- we dynamically update the mathematical bounds: $\text{new\_max} = \max(\text{current\_max}, v.\text{val})$, $\text{new\_min} = \min(\text{current\_min}, v.\text{val})$, and propagate them to disjoint subtrees $T_L$ and $T_R$.
