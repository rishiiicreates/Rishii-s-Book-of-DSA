...
1. **State Invariant:** Maintain `max_level_reached` initialized to $0$. Pass `current_level` (1-indexed) into the recursive function.
2. **Right-First Traversal:** By deliberately exploring `node->right` before `node->left`, we mathematically guarantee that the first time the DFS discovers a specific depth level $L$, it is unconditionally visiting the rightmost node of that level.
3. **Observation Trigger:** Upon reaching `current_level`, check if `current_level > max_level_reached`.
