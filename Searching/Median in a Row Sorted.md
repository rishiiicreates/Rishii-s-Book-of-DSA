...
- the minimum possible element $L$ is the minimum of the first column.
- the maximum possible element $R$ is the maximum of the last column.
- the median of a dataset of size $N = R \times C$ is mathematically defined as the element which has exactly $\lfloor N / 2 \rfloor$ elements strictly smaller than or equal to it (in a 0-indexed sense). So we need an element with at least $\lceil N / 2 \rceil$ elements $\le$ it. Let `req = (R * C) / 2`.
- for a candidate value `mid`, iterate through each row. Since each row is sorted, use `std::upper_bound` to count how many elements are $\le$ `mid`. Sum these counts.
- if `count <= req`, the candidate `mid` is too small to be the median. Search the right half: `low = mid + 1`.
- if `count > req`, `mid` could be the median, or the median is smaller. Search the left half: `high = mid - 1`.
- at the end, `low` will pinpoint the exact median value.
