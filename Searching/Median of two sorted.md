...
- $a[i-1] \le B[j]$ (The largest left element in $A$ is $\le$ the smallest right element in $B$)
- $b[j-1] \le A[i]$ (The largest left element in $B$ is $\le$ the smallest right element in $A$)

- since $A$ and $B$ are sorted, we can binary search the optimal cut $i$ in the smaller array.
- force $A$ to be the smaller array to guarantee $O(\log(\min(N, M)))$ bound and ensure $j \ge 0$.
- initialize search bounds for the cut in $A$: $L = 0$ and $R = N$.
- for $i = L + \lfloor (R - L) / 2 \rfloor$ and $j = K - i$:
