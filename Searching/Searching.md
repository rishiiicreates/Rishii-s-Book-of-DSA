...
- if $A[\text{mid}] == T$, we have found the target.
- if $A[\text{mid}] < T$, the target must be in the right half. Update `left = mid + 1`.
- if $A[\text{mid}] > T$, the target must be in the left half. Update `right = mid - 1`.
