...
- the search space for the optimal height is bounded between $\min(\text{heights})$ and $\max(\text{heights})$.
- for a candidate height `mid`, calculate $f(\text{mid})$ and $f(\text{mid} + 1)$.
- if $f(\text{mid}) < f(\text{mid} + 1)$, we are on the rising slope (right side) of the convex curve. The global minimum must be to the left, so `high = mid - 1`.
- if $f(\text{mid}) \ge f(\text{mid} + 1)$, we are on the falling slope (left side). The global minimum must be to the right, so `low = mid + 1`.
