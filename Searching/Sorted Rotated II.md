...
- algorithm:
- follow the standard deterministic half-validation logic of Rotated Array I.
- if `nums[left] == nums[mid]` AND `nums[mid] == nums[right]`, we are in the indeterministic state.
- to resolve this, we simply shrink the search space linearly by doing `left++` and `right--`.
- this systematically trims the duplicate edges until a deterministic monotonic boundary is revealed.
