...
- **monotonic Deque `max_dq`:** Maintains the indices of the dynamic window such that values are monotonically decreasing, keeping the window's maximum at `max_dq.front()`.
- **dynamic Sliding Window ($L$ and $R$):**
...
- **record Global Maximum:** Update $\text{maxLength} = \max(\text{maxLength}, R - L + 1)$.
