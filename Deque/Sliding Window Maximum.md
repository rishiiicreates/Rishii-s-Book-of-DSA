...
- **initialization:** Initialize an empty `std::deque<int> dq` to store indices, and a vector `res` to store the maximums.
- **iteration:** Loop through the array $A$ from index $i = 0$ to $N-1$.
- **out-of-Bounds Ejection:** Check if the element at the front of the deque has fallen completely out of the current window $[i - K + 1, i]$. If $dq.front() \le i - K$, we mathematically pop it from the front: `dq.pop_front()`.
- **monotonicity Maintenance:** Before inserting the current element $A[i]$, we must ensure the decreasing invariant. While the deque is not empty and the element at the back of the deque is $\le A[i]$, pop it from the back: `dq.pop_back()`.
...
- **insertion:** Push the current index $i$ to the back of the deque: `dq.push_back(i)`.
- **result Harvesting:** Once the window has reached size $K$ (i.e., when $i \ge K - 1$), the index at the absolute front of the deque definitively points to the maximum element for this window. Append $A[dq.front()]$ to the results.
