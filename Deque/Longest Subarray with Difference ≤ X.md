...
2. **Expansion Phase ($R$ moves right):**
...
3. **Contraction Phase ($L$ moves right):**
   ...
   - Increment $L$.
4. **Recording State:** After ensuring the window $[L \dots R]$ is mathematically valid, update the maximum length observed: $\text{maxLength} = \max(\text{maxLength}, R - L + 1)$.
