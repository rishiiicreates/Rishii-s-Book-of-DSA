---
type: concept
tags: [graph, connected-components, dfs, matrix, cpp]
date: 2026-06-30
---
...
1. **Topological Mapping:** The input is provided as an Adjacency Matrix. To optimize edge traversal down to $O(V+E)$, we first convert the $O(V^2)$ matrix into an Adjacency List. *(Note: You can run DFS directly on the matrix, but doing so forces an $O(V)$ scan per node, yielding $O(V^2)$ time even for sparse graphs).*
2. **State Tracking:** Initialize a `visited` array of size $N$, defaulting to `false`. Initialize a scalar counter `provinces = 0`.
