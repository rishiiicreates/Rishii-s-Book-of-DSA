...
- **state Preservation:** We can maintain an $M \times N$ `visited` boolean matrix.
...
- **traversal Constraints:** During BFS/DFS, rigorously validate that neighboring vectors $(nr, nc)$ remain strictly within the mathematical boundaries $0 \le nr < M$ and $0 \le nc < N$, and exclusively target `'1'` cells.
