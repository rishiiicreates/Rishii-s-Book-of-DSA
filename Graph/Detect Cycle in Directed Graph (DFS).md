...
- **state Initialization:** Require two boolean arrays:
...
- **path Reversion:** Before the `dfs(node)` function yields control backward, it is mathematically exiting the path. We **must** reset `pathVisited[node] = false` to remove it from the active topological trace.
