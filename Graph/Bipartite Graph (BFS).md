...
- **state Initialization:** Define an array `color` of size $|V|$ initialized universally to `-1`.
- **disconnected Component Iteration:** Because the graph may disjoint, we loop $0 \dots V-1$. If `color[i] == -1`, trigger `BFS(i)`.
- **bFS Execution:**
...
- if BFS fully exhausts all components without triggering the violation condition, the graph is unconditionally Bipartite. Return `true`.
