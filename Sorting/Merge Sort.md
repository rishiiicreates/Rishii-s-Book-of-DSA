...
- **divide:** Recursively bisect the continuous array $[L, R]$ into two independent halves $[L, M]$ and $[M+1, R]$ until every sub-array has a size of $1$. (A sub-array of size $1$ is mathematically inherently sorted).
- **conquer / Merge:** Recursively merge two sorted sub-arrays into a single strictly sorted macro-array using a deterministic two-pointer approach, maintaining $\mathcal{O}(K)$ time for merging $K$ elements.
