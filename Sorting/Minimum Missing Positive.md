...
1. We want to place each positive integer $x \in [1, N]$ at its correct 0-indexed position: index $x - 1$. For example, $1$ goes to index $0$, $5$ goes to index $4$.
2. Iterate through the array. For the current element $A[i]$, if $1 \le A[i] \le N$ and it is not already at its correct index (i.e., $A[i] \neq A[A[i] - 1]$), we swap $A[i]$ with $A[A[i] - 1]$.
3. Because swapping brings a new element to $A[i]$, we use a `while` loop to continuously resolve the new element until it either falls out of range, is a duplicate, or lands in its correct spot.
4. After resolving all elements, perform a final linear scan. The first index `i` where $A[i] \neq i + 1$ reveals the missing number $i + 1$.
