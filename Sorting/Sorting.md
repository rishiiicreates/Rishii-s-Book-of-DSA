...
- it begins with **Quick Sort** for cache locality and speed.
- it tracks the recursion depth. If the depth exceeds $2 \times \lfloor \log_2 N \rfloor$, Quick Sort is degenerating toward $O(N^2)$ time complexity.
- introsort instantly switches to **Heap Sort** to guarantee an absolute worst-case $O(N \log N)$ bound.
- for extremely small partitions (typically $N < 16$), it defers to **Insertion Sort**, which has optimal instruction cache usage and zero recursive overhead for small scales.
