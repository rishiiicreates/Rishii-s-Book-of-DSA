...
- algorithm:
- maintain two converging boundary pointers: `left` starting at index $0$, `right` starting at index $N-1$.
- the invariant is that everything strictly before `left` is `0`, and everything strictly after `right` is `1`.
- if `arr[left] == 0`, the invariant holds; increment `left`.
- if `arr[right] == 1`, the invariant holds; decrement `right`.
- if `arr[left] == 1` and `arr[right] == 0`, we have found a mathematical inversion. Swap them, increment `left`, and decrement `right`.
- terminate when the boundaries cross (`left >= right`).
