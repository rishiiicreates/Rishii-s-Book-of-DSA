...
- algorithm:
- iterate through the array using a fast scanner pointer `i`.
- if `nums[i] != 0`, it is a valid element that must be shifted leftwards to preserve density.
- swap `nums[i]` with `nums[nonZeroBoundary]`.
- increment `nonZeroBoundary` to secure the element in the dense non-zero block.
- all $0$s are mathematically bubbled to the right side of the boundary.
