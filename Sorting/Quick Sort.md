...
- **pivot Selection:** Select an arbitrary element (e.g., the last element) as the `pivot`.
- **partitioning:** Rearrange the array such that all elements strictly less than the `pivot` are moved to its left, and all elements strictly greater than or equal to the `pivot` are moved to its right. The `pivot` is then mathematically guaranteed to be in its absolute final sorted position.
- **recursion:** Recursively apply this logic to the left sub-array and the right sub-array.
