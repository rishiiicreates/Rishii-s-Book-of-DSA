...
1. **Right-to-Left Traversal:** Traverse from index $N-1$ down to $0$. We process the "future" before evaluating the current element $A[i]$.
2. **Preserving Monotonicity:** While the stack is not empty and the top of the stack is $\ge A[i]$, pop elements.
...
3. **Query Response:** After enforcement, the stack either contains the NSE at its top, or it is empty (implying no such element exists, returning $-1$).
