...
- we evaluate if the newly updated $f(A_i)$ exceeds the global `max_frequency`.
- if the inequality holds, we mathematically overwrite `max_frequency` and register $A_i$ as the current provisional `modal_element`.
