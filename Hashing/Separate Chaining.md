...
- **mapping:** The hash function $h(K)$ maps the key to an index $0 \le \text{index} < M$.
- **insertion:** Calculate `index = h(K)`. Traverse the list at `table[index]` to check for duplicates. If none exist, append $K$ to the list.
- **search/Deletion:** Calculate `index = h(K)`. Traverse the list at `table[index]` to find $K$. This traversal takes time proportional to the length of the chain.
- **expected Length:** If a uniform hash function distributes $N$ elements across $M$ buckets, the expected length of any chain is exactly the load factor $\alpha = \frac{N}{M}$.
