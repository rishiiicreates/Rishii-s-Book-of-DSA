...
- **zero Residue ($R = 0$):** Elements congruent to $0$ must pair exclusively with other elements congruent to $0$. Thus, the frequency $f(0)$ must be exactly **even**.
- **symmetric Residue ($R > 0$):** For any residue $0 < R < K$, the frequency $f(R)$ must exactly equal the conjugate frequency $f(K - R)$.
- **midpoint Residue ($R = K/2$):** If $K$ is even, $R = K/2$ is self-conjugate. Similar to $R=0$, elements congruent to $K/2$ must pair with each other. Thus, $f(K/2)$ must be strictly **even**.
